...
function diem($diem){
    if($diem >=8 && $diem <= 10){
        return '<span class="tuyetvoi">Tuyệt vời</span>';
    }else if($diem >= 7 && $diem <= 7.9){
        return '<span class="rattot">Rất tốt</span>';
    }else if($diem >=5 && $diem <= 6.9){
        return '<span class="binhthuong">Bình thường</span>';
    }else{
        return '<span class="kem">Kém</span>';
    }
}
function getFirstID($uri){
    $uri_str = explode('-',$uri);
    return $uri_str[0];
}
function getLastID($uri){
    $uri_str = explode('-',$uri);
    return $uri_str[(sizeof($uri_str) - 1)];
}
function cv_price($price){
    if($price > 0){
        $price = number_format($price,0,'.','.');
        return substr($price,0,-4).'<span>.000</span>';   
    }else{
        return 'Liên hệ';
    }      
}
...
function js_editor(){
    $js ='<link rel="stylesheet" type="text/css" href="//cdnjs.cloudflare.com/ajax/libs/codemirror/3.20.0/codemirror.min.css" />';
    $js .='<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/codemirror/3.20.0/theme/blackboard.min.css">';
    $js .='<link rel="stylesheet" href="//cdnjs.cloudflare.com/ajax/libs/codemirror/3.20.0/theme/monokai.min.css">';
    $js .='<script type="text/javascript" src="//cdnjs.cloudflare.com/ajax/libs/codemirror/3.20.0/codemirror.js"></script>';
    $js .='<script src="//cdnjs.cloudflare.com/ajax/libs/codemirror/3.20.0/mode/xml/xml.min.js"></script>';
    $js .='<script src="//cdnjs.cloudflare.com/ajax/libs/codemirror/2.36.0/formatting.min.js"></script>';
    return $js;
}

function editor($name, $value, $id){
    $str = '<textarea class="form-control" id="'.$id.'" name="'.$name.'">'.$value.'</textarea>';
    $str .= '<script>';   
    $str .= 'init.push(function () {';   
    $str .= "if (! $('html').hasClass('ie8')) {";   
    $str .= "$('#".$id."').summernote({";   
    $str .= "            height: 300,";
    $str .= "            tabsize: 2,";
    $str .= "            codemirror: {";
