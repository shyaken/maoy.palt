<?php
    ...
    $html = "";
    $get_type = 1;
    $image_types = 'gif|jpg|jpeg|png|flv';
    $upload_dir = '../data/';
    $real_dir = realpath( dirname( __FILE__ ) );
    $base_url = $_SERVER['HTTP_HOST']=='localhost' ? 'http://localhost/luocbao/luocbao2/' : 'http://www.luocbao.com/';
...
function postPage($url,$pvars,$timeout = 60){
    ...
    function _get_image($url, $path_save, $name)
    {
        ...
        if(empty($width) or empty($height) or $type>3) return false;

        $httpcode =_getcontent($url, $this->get_type);
        ...
        $size = file_put_contents($real_dir.'/'.$path_save.$name.$ext, $html);    
        return $size ? $name.$ext : false;
    }
    
    function _read_img($file, $path, $name)
    {
        if(empty($file)) return false;
        list($width, $height, $type, $attr)=@getimagesize($file);
        if(empty($width) or empty($height) or $type>3)
        {            
            $_getimg_error .= ' Can not get image from: '.$file;
            return false;
        }
        else
        {
